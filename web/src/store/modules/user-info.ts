...
const userInfoStore = () => {
  ...
  const wallet_base_types = ref<WalletBaseOption[]>([]);
  ...
  async function setAccount() {
    ...
    if (data && data.data) {
      ...
      wallet_base_types.value = data.data.wallet_base_types || [];
  ...
  return { account, token, setAccount, setToken, logOut, trade_type, trade_fiat, trade_crypto, wallet_base_types, admin_username };
};

export const useUserInfoStore = defineStore("user-info", userInfoStore, {
  persist: persistedstateConfig("user-info", ["token", "account", "trade_type", "trade_fiat", "trade_crypto", "wallet_base_types", "admin_username"])
});
