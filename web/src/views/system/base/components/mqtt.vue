    ...
    <a-col :span="24">
      <a-card title="MQTT 发布设置">
        <a-alert type="info" style="margin-bottom: 16px">
          ...
          <a-link href="https://github.com/chunkburst/PreUSDT/blob/main/docs/api/mqtt.md" target="_blank" :hoverable="false">
            查看文档
          </a-link>
        </a-alert>
        ...
        <a-form :model="form" :rules="rules" :layout="layoutMode" class="base-setting-form" @submit="onSubmit">
          ...
          <a-form-item
            ...
            label="消息路径前缀"
            extra="消息发布的 Topic 路径前缀，只允许字母、数字、下划线和斜杠，默认为 preusdt"
          >
            <a-input v-model="form.mqtt_topic_prefix" placeholder="例如：preusdt" allow-clear />
          </a-form-item>
...
const onSubmit = async ({ errors }: ArcoDesign.ArcoSubmit) => {
  ...
  await setsConfAPI([
    ...
    { key: "mqtt_topic_prefix", value: form.value.mqtt_topic_prefix || "preusdt" }
  ]);
...
watch(
  ...
  () => {
    ...
    form.value.mqtt_topic_prefix = data.value.mqtt_topic_prefix ?? "preusdt";
  }
);
</script>
