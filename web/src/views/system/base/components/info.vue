    ...
    <a-col :span="24">
      <a-card title="基本信息">
        <a-form :model="form" :rules="rules" :layout="layoutMode" class="base-setting-form" @submit="onSubmit">
          ...
          <a-form-item
            field="payment_max_switch_count"
            label="订单切换次数"
            extra="每个待支付订单允许切换支付币种或网络的次数，推荐 3；填 0 或留空时后端会按默认 3 次处理"
          >
            <a-input v-model="form.payment_max_switch_count" placeholder="推荐 3" />
          </a-form-item>
          ...
          <a-form-item field="payment_match_mode" label="金额匹配模式">
            <template #extra>
              ...
              <a-link
                href="https://github.com/chunkburst/PreUSDT/blob/main/docs/payment-match-mode/README.md"
                target="_blank"
                :hoverable="false"
              >
                文档说明
...
const rules = {
  ...
  payment_max_switch_count: [
    {
      required: true,
      type: "number",
      min: 0,
      max: 20,
      message: "订单切换次数必须在0到20之间"
    }
  ],
  payment_match_mode: [
    {
...
const onSubmit = async ({ errors }: ArcoDesign.ArcoSubmit) => {
  ...
  await setsConfAPI([
    ...
    { key: "payment_max_switch_count", value: form.value.payment_max_switch_count },
...
watch(
  ...
  () => {
    ...
    form.value.payment_max_switch_count = data.value.payment_max_switch_count || "3";
