    ...
    <a-spin :loading="loading" tip="正在加载..." class="full-height">
      ...
      <a-card :bordered="false" class="stats-card">
        ...
        <div class="network-stats-grid">
          <div v-for="net in sortedNetworks" :key="net.key" class="network-stat-card" :class="[getNetworkClass(net.key), getNetworkStatus(net.statKey).cardClass]">
            <!-- 网络头部 -->
            <div class="net-card-header">
              ...
              <div class="net-status" :class="getNetworkStatus(net.statKey).className">
                <span class="net-status-dot"></span>
                <span>{{ getNetworkStatus(net.statKey).label }}</span>
              </div>
            </div>
            ...
            <div class="net-card-body">
              ...
              <div class="stat-row">
                ...
                <div class="stat-value block-value">
                  {{ formatBlock(getNetStat(net.statKey)?.block) }}
                </div>
              </div>
      ...
      <a-card :bordered="false" class="main-card">
        <template #title>
          <div class="card-title">
            ...
            <span>区块网络配置</span>
            <a
              href="https://github.com/chunkburst/PreUSDT/blob/main/docs/faq/rpc-endpoint.md"
              target="_blank"
...
const parseSuccRate = (succ?: string): number | null => {
  if (!succ) return null;
  const val = Number.parseFloat(String(succ).replace("%", ""));
  return Number.isFinite(val) ? val : null;
};

const isStaleStat = (stat?: StatInfo): boolean => {
  if (!stat?.time) return false;
  return Math.floor(Date.now() / 1000) - Number(stat.time) > STAT_STALE_SECONDS;
};

const getNetworkStatus = (statKey: string): NetworkStatus => {
  const stat = statsData.value[statKey];
  if (!stat) return { label: "暂无数据", className: "status-unknown", cardClass: "status-card-unknown" };
  if (isStaleStat(stat)) return { label: "未更新", className: "status-stale", cardClass: "status-card-stale" };

  const val = parseSuccRate(stat.succ);
  if (val === null) return { label: "暂无数据", className: "status-unknown", cardClass: "status-card-unknown" };
  if (val >= 95) return { label: "正常", className: "status-good", cardClass: "status-card-good" };
  if (val >= 80) return { label: "不稳定", className: "status-warn", cardClass: "status-card-warn" };
  return { label: "异常", className: "status-bad", cardClass: "status-card-bad" };
};

const getSuccRateClass = (succ?: string) => {
  const val = parseSuccRate(succ);
  if (val === null) return "";
...
const formatBlock = (block?: string): string => {
  ...
  return Number.isFinite(val) ? `#${val.toLocaleString()}` : block;
};
