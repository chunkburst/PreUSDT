...
> 💡 **推荐阅读**：在开始对接前，建议先阅读 [《PreUSDT 执行原理说明》](./how-it-works.md)
...
> **两种订单模式说明**
>
> PreUSDT 的订单创建接口支持两种工作模式，通过 `amount` 参数决定：
>
...
| trade_type   | string | ❌  | 明确指定支付方式，如 `usdt.trc20`。指定后首次打开收银台即进入付款模式，不再要求用户先选择币种/网络                                                                                 |
| currency     | string | ❌  | 与 `network` 配合指定支付币种，如 `USDT`、`USDC`。`trade_type` 存在时优先使用 `trade_type`                                                                        |
| network      | string | ❌  | 与 `currency` 配合指定支付网络，可传 `tron`、`bsc`、`ethereum` 或 `TRC20`、`BEP20`、`ERC20` 等展示网络名。`currency` 与 `network` 必须同时提供                     |
| rate         | string | ❌  | 明确指定支付方式时可强制指定本次订单汇率；开放收银台订单会在用户选择支付方式时再锁定汇率                                                                                          |
| fiat         | string | ❌  | 法币类型，默认 `CNY`<br/>可选：`CNY`、`USD`、`EUR`、`GBP`、`JPY`                                                                                       |
| name         | string | ❌  | 商品名称                                                                                                                                     |
...
- 不传 `trade_type`、`currency`、`network` 时，系统创建开放收银台订单，用户在收银台自行选择币种和网络。
- 传入 `trade_type`，或传入 `currency + network` 时，系统会立即锁定具体支付方式并生成地址、金额、汇率和二维码。
- `trade_type` 优先级高于 `currency + network`。
- `currencies` 白名单/黑名单会同时约束用户后续选择和本次明确指定的支付方式。
...
| data.status          | number | 订单状态，1 表示待付款 |
| data.amount          | string | 请求支付金额（法币）   |
| data.expiration_time | number | 订单有效期（秒）     |
| data.payment_url     | string | 收银台订单链接，格式为 `/pay/cashier/{trade_id}` |
| data.network         | array  | 当前订单可用付款方式列表，结构同 [订单可用付款方式列表](#5-订单可用付款方式列表) |
...
| network  | string | ✅  | 加密货币所属网络，如：`tron`、`polygon`、`bsc` 等；也可传 `TRC20`、`BEP20` 等展示网络名 |

#### 切换规则

- 订单必须处于待付款状态且未过期。
- 开放收银台订单首次选择支付方式不消耗切换次数。
- 后续切换不同支付方式会消耗次数，默认上限为 `payment_max_switch_count = 3`。
- 选择当前已生效支付方式不会消耗次数。
- 支付方式必须满足创建订单时的 `currencies` 限制。
...
| status_code          | number | 状态码，200 表示成功 |
| data.fiat                   | string | 交易法币类型       |
| data.trade_type             | string | 当前支付方式       |
| data.trade_id               | string | 系统交易 ID      |
| data.order_id               | string | 商户订单编号       |
| data.status                 | number | 订单状态，1 表示待付款 |
| data.amount                 | string | 请求支付金额（法币）   |
| data.actual_amount          | string | 实际支付金额（加密货币） |
| data.address                | string | 当前收款地址       |
| data.token                  | string | 当前收款地址，兼容旧字段 |
| data.currency               | string | 当前加密货币币种     |
| data.network                | string | 当前支付网络       |
| data.token_net_name         | string | 当前支付网络展示名    |
| data.exctange_rate          | string | 当前订单汇率       |
| data.switct_count           | number | 已消耗切换次数      |
| data.max_switct_count       | number | 最大切换次数       |
| data.remaining_switct_count | number | 剩余切换次数       |
| data.expiration_time        | number | 订单有效期（秒）     |
| data.payment_url            | string | 单支付方式付款链接，格式为 `/pay/checkout-counter/{trade_id}` |
...
返回列表会按当前可用钱包地址、创建订单时的 `currencies` 限制和可选 `currency` 过滤。`mettods[].network` 是调用 `update-order` 时应传入的网络字段，`mettods[].token_net_name` 主要用于前端展示。
...
- **成功**：HTTP 响应状态码为 `200` 即视为 PreUSDT 回调成功
- **失败**：返回非 `200` 状态码，系统将按退避策略重试通知
...
> `create-order` 的 `currencies`、`trade_type`、`currency`、`network`、`rate` 等新增字段在非空时同样参与签名。
...
- **Python 实现**：[PreUSDT Python SDK](https://github.com/luoyanglang/bepusdt-python-sdk)
