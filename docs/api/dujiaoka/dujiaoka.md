...
由于新版 `PreUSDT` 支持多区块链 USDT、超时回调等新特性，独角数卡自带的 `Epusdt` 插件已无法兼容，**必须替换原有插件**。
...
| 参数名称       | 填写内容                                                      | 说明                                                                        |
|------------|-----------------------------------------------------------|---------------------------------------------------------------------------|
| **商户ID**   | 与 `PreUSDT` 中 `AUTH_TOKEN` 参数保持一致                         | 用于身份验证                                                                    |
