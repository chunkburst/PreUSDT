...
- API 服务地址：`https://your-preusdt.com`
...
在 DuJiao Next 后台中选择渠道类型为 PreUSDT，按照以下格式进行配置。

![PreUSDT 配置界面](./preusdt.png)

**必填配置项**

| 配置项       | 说明                 | 示例                                                 |
|-----------|--------------------|----------------------------------------------------|
| 网关地址      | PreUSDT 的 API 服务地址 | `https://your-preusdt.com`                         |
| API Token | PreUSDT 的 API 密钥   | `your_api_token`                                   |
...
- **支付币种**：详见 [支持的币种列表](https://github.com/chunkburst/PreUSDT/blob/main/docs/trade-type.md)，必须严格区分大小写。
...
- **检查网络连通性**：确认 DuJiao Next API 所在服务器能够正常访问 PreUSDT API 地址。
- **检查 API Token**：确认所配置的 API Token 与 PreUSDT 中的密钥一致。
- **查看日志**：检查 DuJiao Next 和 PreUSDT 的应用日志，获取更详细的错误信息。
