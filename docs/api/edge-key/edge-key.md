# EdgeKey 对接 PreUSDT 教程

**当前教程对接PreUSDT(v1.23.5)通过测试验收**
...
- **App ID 和 App Secret**：在 PreUSDT 后台「系统管理 → 基本设置 → API 设置」中获取。
- **支付币种**：用户在 PreUSDT 收银台页面自由选择支付币种，具体在 PreUSDT 后台配置。
...
**PreUSDT专用字段**：
- **App ID**：PreUSDT后台获取的应用ID
- **App Secret**：PreUSDT后台获取的应用密钥（API密钥）
...
- **检查网络连通性**：确认 EdgeKey 服务器能够正常访问 PreUSDT API 地址。
- **检查 App ID 和 App Secret**：确认所配置的 App ID 和 App Secret 与 PreUSDT 中的一致。
- **检查回调地址**：确保异步通知地址可以从外部访问，且格式正确。
- **检查站点设置**：确保在「站点设置」中配置了正确的网站地址。
- **查看日志**：检查 EdgeKey 和 PreUSDT 的应用日志，获取更详细的错误信息。

## PreUSDT 工作模式

本项目使用 **收银台模式**（`/api/v1/order/create-order`），用户在 PreUSDT 收银台页面自由选择支付链，无需在代码中指定 `trade_type`。

EdgeKey 默认建议保持开放收银台模式：请求中省略 `trade_type`、`currency`、`network`，由用户在 PreUSDT 收银台选择币种和网络；只有确实需要固定某条链时，才在创建订单时明确指定支付方式。

## 相关链接

- [PreUSDT 项目主页](https://github.com/chunkburst/PreUSDT)
