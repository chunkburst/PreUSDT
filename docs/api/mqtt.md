...
PreUSDT 内置了 MQTT 发布能力，每当区块扫描器捕获到一笔**金额合法**的链上转账时，系统会立即将该交易信息以 JSON 格式发布到对应的
MQTT Topic。
...
在 PreUSDT 后台 → **系统管理 → 基本设置 → MQTT 设置** 中填写以下参数：
...
>
> 客户端 ID 由系统自动生成，格式为 `PreUSDT {进程PID}`，每次启动可能不同，请勿依赖客户端 ID 做业务判断。
...
| 网络       | Topic                       | 支持币种                          |
|----------|-----------------------------|-------------------------------|
| Tron     | `preusdt/transfer/tron`     | USDT (TRC20)、USDC (TRC20)、TRX |
| Ethereum | `preusdt/transfer/ethereum` | USDT (ERC20)、USDC (ERC20)、ETH |
| BSC      | `preusdt/transfer/bsc`      | USDT (BEP20)、USDC (BEP20)、BNB |
| Polygon  | `preusdt/transfer/polygon`  | USDT、USDC                     |
| Arbitrum | `preusdt/transfer/arbitrum` | USDT、USDC                     |
| Base     | `preusdt/transfer/base`     | USDC                          |
| Solana   | `preusdt/transfer/solana`   | USDT、USDC                     |
| Aptos    | `preusdt/transfer/aptos`    | USDT、USDC                     |
| X Layer  | `preusdt/transfer/xlayer`   | USDT、USDC                     |
| Plasma   | `preusdt/transfer/plasma`   | USDT                          |

### 订阅示例

```
# 仅订阅 Tron 网络
preusdt/transfer/tron

# 同时订阅 BSC 和 Polygon（分两次 Subscribe）
preusdt/transfer/bsc
preusdt/transfer/polygon
...
PreUSDT 的 MQTT 客户端已启用自动重连，断线后会自动恢复并重新订阅所有 Topic。订阅方同样建议使用支持自动重连的 MQTT 客户端库。
...
> 默认情况下，PreUSDT 仅在有待支付订单时才对对应网络扫块。需在后台**「持续监控网络」**中填入目标网络标识（如 `tron,bsc`
> ），才能保证无订单时也持续推送。
...
- [PreUSDT GitHub](https://github.com/chunkburst/PreUSDT)
