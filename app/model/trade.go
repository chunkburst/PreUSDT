...
func SwitchOrderPayment(t Order, p OrderParams, initialSelection bool, maxSwitch int) (Order, error) {
	if p.TradeType == t.TradeType && p.Money.String() == t.Money && p.Fiat == t.Fiat && t.Address != "" && t.Amount != "0" {
		return t, nil
	}
	...
	where := Db.Model(&Order{}).Where("trade_id = ? AND status = ? AND expired_at > ?", t.TradeId, OrderStatusWaiting, time.Now())
	if initialSelection {
		where = where.Where("(address = ? OR amount = ?)", "", "0")
	} else {
		updates["switch_count"] = gorm.Expr("switch_count + ?", 1)
		where = where.Where("switch_count < ?", maxSwitch)
	}

	res := where.Updates(updates)
	if res.Error != nil {
		return t, res.Error
	}
	if res.RowsAffected == 0 {
		return t, errors.New("订单状态已变化、已过期或切换次数已用完")
	}

	updated, ok := GetTradeOrder(t.TradeId)
	if !ok {
		return t, errors.New("order not found")
	}
