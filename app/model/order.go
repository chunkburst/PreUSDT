...
type Order struct {
	...
	SwitchCount   int        `gorm:"column:switch_count;not null;default:0;comment:支付方式切换次数" json:"switch_count"`
...
func (o *Order) MarkConfirmingIfCurrent(blockNum int, from, hash string, at time.Time, amount decimal.Decimal) bool {
	...
	where := Db.Model(&Order{}).
		Where("trade_id = ?", o.TradeId).
		Where("status = ?", OrderStatusWaiting).
		Where("trade_type = ?", o.TradeType).
	...
	if o.AddressLocked {
		...
		where = where.Where("amount = ?", o.Amount)
	}
