...
import (
	...
	epay2 "github.com/chunkburst/PreUSDT/app/handler/epay"
...
func epay(ctx context.Context, order model.Order) error {
	...
	postReq.Header.Set("Powered-By", "https://github.com/chunkburst/PreUSDT")
...
func epusdt(ctx context.Context, order model.Order) error {
	...
	postReq.Header.Set("Powered-By", "https://github.com/chunkburst/PreUSDT")
	postReq.Header.Set("User-Agent", "PreUSDT/"+app.Version)
	resp, err := client.Do(postReq)
	if err != nil {
...
func PreUSDT(o model.Order) {
	...
	var client = &http.Client{Timeout: time.Second * 5}
	go func() {
		if err := deliverPreUSDTStatusUpdate(model.Db, client, authToken, o); err != nil {
			log.Warn("notify PreUSDT Error:", err.Error())
		}
	}()
}

func deliverPreUSDTStatusUpdate(db *gorm.DB, client *http.Client, authToken string, o model.Order) error {
	if client == nil {
	...
	req.Header.Set("Powered-By", "https://github.com/chunkburst/PreUSDT")
