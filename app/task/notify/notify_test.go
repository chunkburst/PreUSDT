...
import (
	...
	applog "github.com/chunkburst/PreUSDT/app/log"
...
func TestDeliverPreUSDTStatusUpdateDoesNotHoldDBWhileHTTPIsPending(t *testing.T) {
	...
	go func() {
		errCh <- deliverPreUSDTStatusUpdate(db, &http.Client{Timeout: 2 * time.Second}, "test-auth-token", order)
	}()
