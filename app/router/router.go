...
func Handler() *gin.Engine {
	...
	engine.GET("/", func(ctx *gin.Context) {
		...
		homepageHTML := model.GetK(model.HomepageTemplateHTML)
		homepageCSS := model.GetK(model.HomepageTemplateCSS)
		if homepageHTML != "" && utils.IsSafeHomepageHTML(homepageHTML) && utils.IsSafeHomepageCSS(homepageCSS) {
			page := `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    ...
    <link rel="icon" href="/payment/assets/img/favicon.ico" type="image/x-icon">
    <title>{{ .title }}</title>
    <style>{{ .css }}</style>
</head>
<body>
` + homepageHTML + `
			...
			tmpl, err := template.New("homepage-custom").Parse(page)
			if err == nil {
				var buf bytes.Buffer
				err = tmpl.Execute(&buf, gin.H{"title": conf.Desc, "url": conf.Github, "css": template.CSS(homepageCSS)})
				if err == nil {
...
func copyright() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		ctx.Writer.Header().Set("Payment-Gateway", "https://github.com/chunkburst/PreUSDT")
	}
}
