...
func IsSafeHomepageHTML(raw string) bool {
	if len(raw) > 20000 {
		return false
	}
...
func IsSafeHomepageCSS(raw string) bool {
	if len(raw) > 20000 {
		return false
	}
...
func IsSafeTelegramTemplate(raw string) bool {
	if len(raw) > 4000 {
		return false
	}

	return !strings.ContainsRune(raw, rune(0))
}
